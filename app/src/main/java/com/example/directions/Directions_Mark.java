...
public class Directions_Mark {
    ...
    public static void markReturn(Bitmap markerImage, TMapView tMapView, TMapPoint[] markerItemPoint, String[] title, int buttonNum, int[] buttonClickSize, ArrayList<Integer> buttonNumArr){
        ...
        markerItem1.setCalloutSubTitle(title[buttonNum]); // 풍선뷰의 서브제목을 받은 Title값을  표시해줍니다.
        markerItem1.setCalloutTitle(buttonNum + "번째 장소"); // 풍선뷰의 제목을 받은 숫자값으로 표시해줍니다.
    ...
    public static void allMarkReturn(Bitmap markerImage, TMapView tMapView, TMapPoint[] markerItemPoint, String[] title, int buttonNum, int[] buttonClickSize, ArrayList<Integer> buttonNumArr) {
        for(int i = 0; i < buttonNumArr.size(); i++) {
            ...
            markerItem1.setCalloutSubTitle(array_saving_class.nameOfIt[buttonNumArr.get(i)]); // 위치 자동으로 찍히게 수정해야함 -> 수정완료
            markerItem1.setCalloutTitle(Integer.toString(buttonNumArr.get(i)) + "번째 장소");
    ...
    public static void markReturn(TMapPoint[] alTMapPoint, String[] nameOfIt, int buttonNum, int[] btnClickSize, ArrayList<Integer> buttonNumArr) {
    }
}